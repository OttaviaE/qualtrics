...
## Genere, generazione, tempi di risposta (SD)
...
temp.item$item.diff = factor(temp.item$item.diff, 
                             levels = levels(item_char_scol$item.diff))

temp.item[order(temp.item$item.diff), ] %>% 
  kable(align="c")
...
A livello generale si ha questa situazione rispetto ai tempi di risposta: 
...
Calcolo la differenza nei tempi di risposta tra i livelli estremi delle generazioni, ossia (per il momento tra old.gen e gen.z). La differenza è calcolata tra old gen e gen z, per cui valori positivi indicano che old generation è più lenta della gen.z
