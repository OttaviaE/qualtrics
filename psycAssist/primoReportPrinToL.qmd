...
data = read.csv("D:/PRIN2020/qualtrics/psycAssist/ToL/tol43_2023_02_24.csv", 
                header = T, sep = ",")
...
temp$task_success = ifelse(temp$task_success == "VERO", 
                           1, 0)
...
d1$task_success = ifelse(d1$task_success == "FALSO",
                         "error", "giusto")
...
only_correct_p = correct_p[correct_p$response %in% "giusto", ]
...
correct_i = correct_i[correct_i$response %in% "giusto", ]
...
```{r}
d.correct = d1[d1$task_success %in% "giusto", ]
...
#item_correct_scol=item_correct_scol[item_correct_scol$response %in% "giusto", ]
...
ggplot(item_correct_scol, 
  ...
  geom_bar(stat = "identity", position = position_dodge()) + 
  facet_wrap(~anni_scolarita) + 
  ggtitle("Proporzione risposte corrette per tipologie di problema") + 
  theme_light()
```
